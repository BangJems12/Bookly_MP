    ...
    try {
      ...
      if (user == null || user.id.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("User belum login, tidak bisa upload bukti transfer.")),
        );
      ...
      final fileName = "${bankPrefix}_${DateTime.now().millisecondsSinceEpoch}_${_selectedImage!.name}";
      await supabase.storage.from('Buktitransfer').uploadBinary(fileName, bytes);
      final fileUrl = supabase.storage.from('Buktitransfer').getPublicUrl(fileName);
      ...
      // ✅ Update status peminjaman langsung ke History
      await supabase
          .from('peminjaman')
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Bukti transfer berhasil diupload & status masuk History!")),
      );
        ...
        children: [
          ...
          Card(
            ...
            child: ListTile(
              ...
              title: const Text("Total Pembayaran", style: TextStyle(fontWeight: FontWeight.bold)),
              subtitle: Text(
                hargaDisplay,
          ...
          Card(
            ...
            child: ListTile(
              ...
              title: const Text("Rekening Tujuan", style: TextStyle(fontWeight: FontWeight.bold)),
              subtitle: Text(
